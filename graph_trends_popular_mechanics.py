...
class PopMechTrendGrapher:
    ...
    def strain_articles(self, lst_of_articles):
        ...
        for art in lst_of_articles:
            ...
            if "The Blood of the Crab" in art.title:
                print("Not including", art)
                continue
    ...
    def graph_label_popularities(self, lst_of_articles):
        ...
        '''
        labels = {
            "Animals": [], "Archaeology": [], "Earth": [], "Energy": [],
            "Green Tech": [], "Health": [], "Math": [], "Science": []
        }
        ...
        trace1 = go.Scatter(x=([pair[0] for pair in averages]), y=([pair[1] for pair in averages]),
                            marker={'color': 'purple', 'symbol': 'dot', 'size': "7"}, mode="lines",
                            text='', name='Popularity - days of the week')

        data = go.Data([trace1])

        layout = go.Layout(autosize=True, title="Third Plot", xaxis={"autorange": True, 'title': '', },
    ...
    def graph_label_date_trends(self, lst_of_articles, graph_avgs=True):
        ...
        for i, key in enumerate(labels.iterkeys()):
            ...
            traceLine = go.Scatter(x=(unique_date_points), y=(y_averages), showlegend=False,
                                marker={'color': colors[i], 'symbol': 'dot', 'size': "7"}, mode="markers+lines",
                                text='', name=None)
        ...
        layout = go.Layout(autosize=True, title="Popular Mechanics Popularity Trends", xaxis={"autorange": True, 'title': '', },
                           yaxis={"autorange": True, 'title': 'Popular Mechanics Popularity Trends'})
    ...
    def main(self, filename, graph_choice):
        ...
        choices = [
                     self.plot(self.graph_label_popularities(self.list_of_articles), 'py_graph')
                     self.plot(self.graph_label_date_trends(self.list_of_articles, graph_avgs=False), 'py_graph'),
                  ]

        return choices[graph_choice]
def main():
    grapher = PopMechTrendGrapher()
    grapher.main("pop_mech_articles0.dat")